...
class MCP23017 : public I2cBoard {
    ...
    private:
        ...
        bool button_states[16] = {false};
        const int DEBOUNCE_TIME = 50;
        int debounce_timers[16] = {0};
};

class ADS1015 : public I2cBoard {
    public:
        ADS1015(String name, int id, std::array<int, 4> axisBindings);

        void initialize();
        void update();
    
    private:
        Adafruit_ADS1X15 board;
        std::array<int, 4> pinAxisBindings; //which DInput axis each pin is bound to
};

class ADS7830 : public I2cBoard {
    public:
        ADS7830(String name, int id, std::array<int, 8> axisBindings);
